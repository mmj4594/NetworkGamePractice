...
void Ball::reset()
{
	...
	{
		setPosition(glm::vec2(Game::Get().lastRoundWinnerPlayerID == 1 ? Config::Get().INITIAL_BALL_POSITION.x : Config::Get().SCREEN_WIDTH - Config::Get().INITIAL_BALL_POSITION.x, Config::Get().INITIAL_BALL_POSITION.y));
	}
}
